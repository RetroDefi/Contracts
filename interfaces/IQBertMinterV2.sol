...
interface IQBertMinterV2 {
    function isMinter(address) view external returns(bool);
    function amountQBertToMint(uint bnbProfit) view external returns(uint);
    function amountQBertToMintForQBertBNB(uint amount, uint duration) view external returns(uint);
    ...
    function mintForQBertBNB(uint amount, uint duration, address to) external;

    function qbertPerProfitBNB() view external returns(uint);
    ...
    function safeQBertTransfer(address to, uint256 amount) external;
