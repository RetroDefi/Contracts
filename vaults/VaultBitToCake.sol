...
contract VaultBitToCake is VaultController, IStrategy, RewardsDistributionRecipientUpgradeable, ReentrancyGuardUpgradeable {
    ...
    PoolConstant.PoolTypes public constant override poolType = PoolConstant.PoolTypes.BitToCake;
    ...
    function _deposit(uint amount, address _to) private nonReentrant notPaused updateReward(_to) {
        require(amount > 0, "VaultBitToCake: amount must be greater than zero");
    ...
    function setRewardsToken(address newRewardsToken) public onlyOwner {
        require(address(_rewardsToken) == address(0), "VaultBitToCake: rewards token already set");
    ...
    function _notifyRewardAmount(uint reward) private updateReward(address(0)) {
        ...
        require(rewardRate <= _balance.div(rewardsDuration), "VaultBitToCake: reward rate must be in the right range");
    ...
    function setRewardsDuration(uint _rewardsDuration) external onlyOwner {
        require(periodFinish == 0 || block.timestamp > periodFinish, "VaultBitToCake: reward duration can only be updated after the period ends");
    ...
    function recoverToken(address tokenAddress, uint tokenAmount) external override onlyOwner {
        require(tokenAddress != address(_stakingToken) && tokenAddress != _rewardsToken.stakingToken(), "VaultBitToCake: cannot recover underlying token");
